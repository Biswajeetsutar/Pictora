...
const loadRazorpay = () => {
  return new Promise((resolve) => {
    if (
      ...
      return resolve(true); // If already loaded, resolve immediately
    }
...
const BuyCredit = () => {
  ...
  const initPay = async (order) => {
    ...
    if (!isLoaded) {
      ...
      toast.error("Payment failed: Razorpay SDK not loaded.");
      return;
    }

    if (!window.Razorpay) {
      console.error("Razorpay is not available.");
      toast.error("Payment failed: Razorpay not available.");
      return;
    }
    ...
    try {
      ...
      toast.error("Payment failed: Razorpay initialization error.");
    }
  };
